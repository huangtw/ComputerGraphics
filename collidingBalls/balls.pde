...
void loadBALLS(String fn) {
  ...
  float ballToBallCollisionTime(int i, int j, float m) {
    float t = Float.MAX_VALUE;
    vec dv = V(m, M(V[i], V[j]));
    vec dp = V(G[j], G[i]);
    ...
    if(a != 0) {
      float b = 2 * dot(dv, dp);
      float c = dot(dp, dp) - (r[i] + r[j]) * (r[i] + r[j]);
  
      float tmp = b * b - 4 * a * c;
      //println("b^2-4ac = " + tmp + ", a = " + a + ", b = " + b + ", c = " + c);
      float tmp_t;
      if(tmp >= 0) {
        tmp = sqrt(tmp);
        //println("tmp = " + tmp);
  ...
  float ballToWallCollisionTime(int i, float m) {
    float t_x, t_y, t_z;
    t_x = t_y = t_z = Float.MAX_VALUE;

    if(V[i].x > 0) {
      t_x = (w / 2 - r[i] - G[i].x) / (m * V[i].x); 
    }
    else if(V[i].x < 0) {
      t_x = ((-w / 2) + r[i] - G[i].x) / (m * V[i].x);
    }
  ...
  float findNextCollisionTime(float m) {
    float nextCollisionTime = Float.MAX_VALUE;
    float t;
    int next_i, next_j;
    ...
    for(int i = 0; i < nv - 1; i++) {
      for(int j = i + 1; j < nv; j++) {
        t = ballToBallCollisionTime(i, j, m);
        if(t < nextCollisionTime) {
          nextCollisionTime = t;
          next_i = i;
          next_j = j;
        }
