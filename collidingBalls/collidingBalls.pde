/*
By Tzu-Wei Huang (GT ID: 903164794)
I implemnted BALLS.ballToBallCollisionTime and BALLS.ballToWallCollisionTime
to find the next collision time between two balls and the next collision time between a ball and the wall.
Then, I implemnted BALLS.findNextCollisionTime to find the earliest collision time.
In BALLS.findNextCollisionTime, the color of balls involved in the first collision is changed to yellow.
*/
...
void setup() {
  ...
  firstT = P.findNextCollisionTime(mv);
  currentT = 0;
  F = P();
  sphereDetail(6);
  frameRate(Frate);
...
//   fill(magenta); noStroke(); show(pick(mouseX,mouseY),10);
  ...
  scribe("dt01 = "+nf(dt01,2,1)+"%, dt12 = "+nf(dt12,2,1)+"%, dt23 = "+nf(dt23,2,1)+"%, dt34 = "+nf(dt34,2,1)+"%"+"currentT = "+currentT+",firstT = "+firstT,10,60); 
  if(filming && (animating || change)) saveFrame("FRAMES/F"+nf(frameCounter++,4)+".png");  // save next frame to make a movie

  if(!stop && firstT - currentT <= 1) {
    stop = true;
  }
...
void keyPressed() {
  ...
  if(key=='i') {P.initPointsOnGrid(nbs,w,br,cyan); firstT = P.findNextCollisionTime(mv); currentT = 0; stop=false;}
...
String title ="6491-2015-P1: Animation of colliding balls", name ="Tzu-Wei Huang",
